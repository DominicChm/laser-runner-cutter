...
def marshal_returnable_to_idl(
    ...
    def _marshal(args, kwargs):
        ...
        try:
            ...
            raise ValueError(
                f"Could not marshal returnable >{returnable}< into >{idl}<"
            )

    # Handle returnable helpers
    if isinstance(returnable, Returnable):
        if (enforce_tag is not None) and returnable.tag != enforce_tag:
            raise PreMarshalError(
                f"Returnable type does not match the enforced returnable type >{enforce_tag}<"
            )
    ...
    # Handle raw, user-constructed IDL returns
        ...
        if not isinstance(returnable, idl):
            raise PreMarshalError(
                f"Passed IDL object >{returnable}< does not match the required IDL >{idl}<!"
            )
    ...
    # Handle dict inputs helpers
        ...
        if enforce_tag is not None:
            raise PreMarshalError(
                f"Cannot marshal from a dictionary when a tag (>{enforce_tag}<) is enforced. Pass either an IDL instance or the appropriate returnable."
            )
        ...
        raise ValueError(
            f"Non-returnable, non-idl, non-dict value >{returnable}< returned/yeilded! This function must return a dict, and IDL, or a `Returnable`!"
        )
